...
- Two Notensysteme coexist: `sechser` (1–6, lower is better, Unter-/Mittelstufe) and `punkte` (Notenpunkte 0–15, higher is better, Kursstufe). A Halbjahr fixes its Notensystem; never mix Notensysteme inside a Halbjahr. Cross-system comparison follows the official Notentendenz anchors (`1+` = 15, `1` = 14, `1-` = 13, `2+` = 12, and so on) using the app's quarter-grade values and linear interpolation only between anchors. The conversion lives in `src/shared/noten/` and is the only place allowed to encode it.
- Fachgewichtung is teacher-declared and stored as one `weighting` jsonb column: an optional schriftlich:mündlich ratio kept in the scale it was announced in, plus per-Leistungsart Gewicht and Sammlung (`gesammelt` averages all Noten of one Leistungsart into a single Note, which is what makes "alle Tests zusammen zählen wie eine Klausur" true regardless of how many Tests there are; the form offers that choice on Test only, because that is the only place a teacher announces it). Rules such as "eine GFS zählt wie eine Klausur" are derived from equal Artgewichtungen, never stored. The Wertungsbereich is a property of the Leistungsart, not a stored per-Fach choice: `bereichDerLeistungsart` fixes Klausur, GFS and Test to schriftlich and Mündlich and Sonstige to mündlich. Teachers announce the ratio between the Wertungsbereiche, never which Leistungsart occupies which — so there is no Wertungsbereich picker, and a Test can never count mündlich. Zeugnis previews round: Halbjahr to quarter steps (sechser) or whole Notenpunkte, Jahreszeugnis to whole Noten with an explicit Grenzfall flag. These rules are load-bearing product logic — change only with tests.
