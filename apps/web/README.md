...
Gelöscht werden kann ein Halbjahr nur, solange keine Note daran hängt — der versehentlich angelegte Eintrag verschwindet, ein belegtes Halbjahr bleibt. `listHalbjahre` führt die Notenanzahl deshalb mit, sodass die Liste das Löschen beim belegten Halbjahr gar nicht erst anbietet. Verbindlich ist die Prüfung im Dienst: Sie sperrt die Zeile und zählt erneut, bevor gelöscht wird, und ist der einzige Löschweg — der Fremdschlüssel der Noten kaskadiert zwar, entscheidet aber nie über sie. Verliert ein Schuljahr dabei sein letztes Halbjahr, wird auch sein fixierter Fachstand freigegeben; ein später erneut angelegtes Schuljahr leitet damit wieder vom dann aktuellen Vorjahr ab, statt einen verwaisten Stand zu erben.

## Fach- und Zeugnislogik

Die Gewichtung eines Fachs bildet die Verkündung der Lehrkraft ab und liegt als eine `weighting`-jsonb-Spalte vor; gelesen wird sie ausschließlich über `dekodiereGewichtung` in `src/shared/noten/fach-gewichtung.ts`. Sie besteht aus dem Bereichsverhältnis schriftlich zu mündlich — im verkündeten Maßstab gespeichert, also „3:1“ und „60:40“ gleichermaßen, oder `null` für eine gemeinsame Liste — und je Leistungsart aus Gewicht und Sammlung. `sammlung: 'gesammelt'` mittelt alle Noten einer Art zu einer einzigen Note, wodurch „alle Tests zusammen zählen wie eine Klausur“ unabhängig von der Anzahl der Tests gilt; das Formular bietet diese Wahl nur beim Test an, weil nur dort eine Lehrkraft sie verkündet. Regeln wie „eine GFS zählt wie eine Klausur“ werden nicht gespeichert, sondern aus der Gleichheit zweier Artgewichtungen abgeleitet. Welchem Bereich eine Note zugerechnet wird, entscheidet weder die Note noch das Fach, sondern die Leistungsart: `bereichDerLeistungsart` in `src/shared/noten/notenwert.ts` stellt Klausur, GFS und Test fest auf schriftlich, Mündlich und Sonstige fest auf mündlich. Verkündet wird das Verhältnis der Bereiche, nicht ihre Besetzung.
