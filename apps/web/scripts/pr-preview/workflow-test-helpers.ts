...
export const runSelection = (
  ...
  const harness = `
set -u
test_root=$(mktemp -d /tmp/punktlandung-preview-test.XXXXXX)
...
case "$endpoint" in
  ...
  */actions/workflows/318356063/runs*) printf '%s' "$SELECT_CANDIDATES" ;;
...
status=$?
set -e
printf '\n__PUNKTLANDUNG_OUTPUTS__\n'
cat "$GITHUB_OUTPUT"
printf '__PUNKTLANDUNG_LOG__\n'
cat "$SELECT_GH_LOG"
printf '__PUNKTLANDUNG_END__\n'
exit "$status"
`;
  ...
  const outputText = stdout
    .split('__PUNKTLANDUNG_OUTPUTS__\n', 2)[1]
    ?.split('__PUNKTLANDUNG_LOG__\n', 1)[0];
  const log = stdout
    .split('__PUNKTLANDUNG_LOG__\n', 2)[1]
    ?.split('__PUNKTLANDUNG_END__\n', 1)[0];
